...
class CloudEventsFunctionInvocationHelper implements FunctionInvocationHelper<Message<?>>, ApplicationContextAware {
	...
	@Override
	public boolean isRetainOuputAsMessage(Map<String, Object> messageHeaders) {
	...
	public Message<?> preProcessInput(Message<?> input, Object inputConverter) {
		return CloudEventMessageUtils.toBinary(input, (MessageConverter) inputConverter);
	}

	@Override
	public Message<?> postProcessResult(Message<?> input, Object result) {
		CloudEventAttributes mutableAttributes = CloudEventAttributeUtils.toAttributes(input.getHeaders());
		String prefix = "";
		if (((MutableCloudEventAttributes) mutableAttributes).isValidCloudEvent()) {
			...
			((MutableCloudEventAttributes) mutableAttributes)
					.setSource(URI.create("http://spring.io/" + getApplicationName()));
			((MutableCloudEventAttributes) mutableAttributes).setType(result.getClass().getName());
				...
				cloudEventAttributesProvider = new CloudEventAttributesProvider() {
					...
					public CloudEventAttributes getOutputAttributes(CloudEventAttributes attributes) {
						if (attributes instanceof MutableCloudEventAttributes) {
							return ((MutableCloudEventAttributes) attributes).setId(UUID.randomUUID().toString())
									.setSource(URI.create("http://spring.io/" + getApplicationName()))
			...
			prefix = determinePrefixToUse(input.getHeaders());
		}
		...
		return MessageBuilder.withPayload(result)
				.copyHeaders(((MutableCloudEventAttributes) mutableAttributes).toMap(prefix)).build();
	}
